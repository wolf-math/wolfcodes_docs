...
---
While loops repeat code as long as a condition remains true. Unlike [for loops](/python/guides/control_flow/for_loops) that iterate over a fixed collection, while loops continue until their condition becomes false, making them perfect for situations where you don't know in advance how many iterations you'll need.
...
- **[For loops](/python/guides/control_flow/for_loops)**: When you know what you're iterating over ([lists](/python/guides/data_structures/lists), [ranges](/python/guides/data_structures/ranges), etc.) or how many times to repeat
...
The [`isdigit`](/python/language_reference/string#isdigit) method checks if a string can be cast into a number. If `user_input.isdigit()` returns `True`, then the user input a number, otherwise the loop repeats. 
