...
It shares many mathematical similarities with [Diffie-Hellman](/projects/encryption/diffie-hellman-key-exchange) encryption, but it's conceptually a little different. Instead of thinking in terms of shared secrets, RSA can be thought of as sending an unlocked padlock. Anyone can use it to lock a message, but only the owner of the key can unlock it.
